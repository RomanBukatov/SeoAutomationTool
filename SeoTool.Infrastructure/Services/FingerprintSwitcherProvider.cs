...
{
    public class FingerprintSwitcherProvider : IFingerprintProvider
    {
        private readonly HttpClient _httpClient;
        private readonly string _apiKey;
        private const string ApiUrl = "https://api.fingerprintswitcher.com/v1/fingerprints";

        public FingerprintSwitcherProvider(HttpClient httpClient, string apiKey)
        {
        ...
        public async Task<Fingerprint> GetFingerprintAsync()
        {
            try
            {
                ...
                // Предполагаем, что API возвращает JSON с полем "fingerprint" или "value"
            ...
            {
                throw new InvalidOperationException($"Failed to get fingerprint from API: {ex.Message}", ex);
            }
